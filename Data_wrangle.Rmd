...
#make bray curtis data matrix
  ...
  samp.DIV_RelA.dist <- samp.DIV_RelA.dist[, -c(1:11,13:17, 19:20,22:24, 26:27, 29:30)] #remove unneeded cols
```
Adonis2
```{r}
bc <- phyloseq::distance(DIV_RelA, method = "bray")
 data <- as(sample_data(DIV_RelA), "data.frame")
 vegan::adonis2(bc ~ SampleNum, data = data)
#Permutation test for adonis under reduced mod

#this output is not correct. it says Model and not TagID
 set.seed(30)
  PERMANOVA<-vegan::adonis2(DIV_RelA.dist ~ TagID,data = samp.DIV_RelA.dist) 
  PERMANOVA
summary(PERMANOVA)

#trying location nested, also weird Model output
PERMANOVA <- vegan::adonis2(DIV_RelA.dist ~ TagID / Location, data = samp.DIV_RelA.dist)
PERMANOVA
summary(PERMANOVA)

#This gives the right TagID output but unsure I believe it is working
adonis2(DIV_RelA.dist ~ TagID, data = samp.DIV_RelA.dist, by = "margin")
adonis2(DIV_RelA.dist ~ TagID * Location, data = samp.DIV_RelA.dist, by = "margin")
